...
(function () {
  ...
  /* ---- News article: Hero slideshow ---- */
  var slideshow = document.getElementById('heroSlideshow');
  if (slideshow) {
    var slides = slideshow.querySelectorAll('.news-slideshow-img');
    var thumbs = slideshow.querySelectorAll('.news-slideshow-thumb');
    var currentSlide = 0;
  ...
  if (newsGrid) {
    var newsCatBtns = document.querySelectorAll('.news-cat-btn');
    var newsCards = newsGrid.querySelectorAll('.news-card');
    ...
    newsCatBtns.forEach(function (btn) {
      btn.addEventListener('click', function () {
        activeCategory = btn.getAttribute('data-category');
        newsCatBtns.forEach(function (b) {
          b.classList.remove('active');
          b.setAttribute('aria-selected', 'false');
        });
        btn.classList.add('active');
    ...
    var newsCatSelect = document.getElementById('news-cat-mobile');
    if (newsCatSelect) {
      newsCatSelect.addEventListener('change', function () {
        activeCategory = newsCatSelect.value;
        newsCatBtns.forEach(function (b) {
          b.classList.toggle('active', b.getAttribute('data-category') === activeCategory);
          b.setAttribute('aria-selected', b.getAttribute('data-category') === activeCategory ? 'true' : 'false');
        });
        applyNewsFilter();
      });
    }
  ...
  var slideshow = document.getElementById('articleSlideshow');
  if (slideshow) {
    var slides = slideshow.querySelectorAll('.news-slide');
    var thumbs = slideshow.querySelectorAll('.news-thumb');
    var cur = 0;
    var timer;
    ...
    function goTo(idx) {
      ...
      // Scroll active thumb into view (horizontal only, don't move the page)
      var thumbsContainer = slideshow.querySelector('.news-slideshow-thumbs');
      var thumb = thumbs[cur];
      var scrollLeft = thumb.offsetLeft - thumbsContainer.offsetWidth / 2 + thumb.offsetWidth / 2;
    ...
    function resetTimer() {
      ...
      timer = setInterval(function () {
        if (isVisible()) goTo(cur + 1);
      }, 5000);
    }

    slideshow.querySelector('.news-slideshow-prev').addEventListener('click', function () { goTo(cur - 1); });
    slideshow.querySelector('.news-slideshow-next').addEventListener('click', function () { goTo(cur + 1); });
